...
void CircularKeyboard::submitInput()
{
...
{
    // USB keyboard: printable character typed directly — append to input
    const QString text = event->text();
    if (!text.isEmpty() && text.at(0).isPrint()) {
        if (m_maxLength < 0 || userInputString.length() < m_maxLength) {
            userInputString += text.at(0);
    ...
    case Qt::Key_Return:
    case Qt::Key_Enter:
        // No hardware path generates Key_Return — it only comes from a USB keyboard.
        submitInput();
        break;
    ...
    case Qt::Key_Escape:
        ...
        break;
    case Qt::Key_A:
        // Key_A with empty text = synthesized hardware submit button.
        // Key_A with text = the letter 'a'/'A', already handled above by the printable path.

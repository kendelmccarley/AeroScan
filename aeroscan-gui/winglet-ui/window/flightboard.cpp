    ...
    : QWidget{parent}
{
    ...
    connect(WingletGUI::inst->adsbReceiver, SIGNAL(newAircraftAdded()), this, SLOT(getAirTraffic()));
    tir->start(1000);
...
void FlightBoard::getAirTraffic() {
    ...
    QList<Aircraft> sortedAircraft = airspace.values();
    std::sort(sortedAircraft.begin(), sortedAircraft.end(), [](const Aircraft &a, const Aircraft &b) {
        return a.timestamp > b.timestamp;
    });

    int row = 0;
    for (const Aircraft &entry : sortedAircraft) {

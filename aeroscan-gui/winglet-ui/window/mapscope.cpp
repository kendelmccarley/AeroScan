...
bool MapScope::setMapTile(double zoom, double lat, double lon){
    ...
    // background fill shows through where there is no map content.
    auto brightenTile = [](QImage *img) {
        if (img->format() != QImage::Format_ARGB32)
            *img = img->convertToFormat(QImage::Format_ARGB32);
        constexpr int boost = 60;
        for (int y = 0; y < img->height(); ++y) {
            QRgb *line = reinterpret_cast<QRgb *>(img->scanLine(y));
            for (int x = 0; x < img->width(); ++x) {
                const QRgb p = line[x];
                if (qAlpha(p) == 0) continue;
                line[x] = qRgba(qMin(255, qRed(p)   + boost),
                                qMin(255, qGreen(p) + boost),
                                qMin(255, qBlue(p)  + boost),
                                qAlpha(p));
            }
        }
    };

...
static const int ZOOM_RANGES_NM[2]   = {15, 5};
    ...
    : QWidget{parent}
{
    ...
    QObject::connect(zoomDebounceTimer, &QTimer::timeout, this, &RadarScope::processDebouncedZoom);
//    this->resize(480, 480);
...
void RadarScope::wheelEvent(QWheelEvent *wheelEvent) {
    ...
    if (this->viewMap == false){
        if(wheelEvent->angleDelta().y() > 0) {
            if (zoomPresetIdx > 0) { zoomPresetIdx--; applyZoomPreset(); }
        } else if(wheelEvent->angleDelta().y() < 0) {
            if (zoomPresetIdx < 1) { zoomPresetIdx++; applyZoomPreset(); }
        }
    }else {
...
bool RadarScope::updateMap(bool doForce){
    ...
    if (diff_lat != 0.0f || diff_lon != 0.0f) {
        status = setMapTile(this->mapLevel, currentGPS.latitude, currentGPS.longitude);
        if(status == true){
            haveMapTiles = true;
        }else{
...
bool RadarScope::setMapTile(double zoom, double lat, double lon){
...
void RadarScope::applyZoomPreset()
{
    pixel_to_miles = 240.0f / ZOOM_RANGES_NM[zoomPresetIdx];
    ring_range_miles = ZOOM_RANGES_NM[zoomPresetIdx];
}

void RadarScope::processDebouncedZoom()
{

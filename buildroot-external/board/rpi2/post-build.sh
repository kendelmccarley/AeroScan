...
# ─── Framebuffer console login (tty1) ────────────────────────────────────────
mkdir -p "${TARGET_DIR}/etc/systemd/system/getty.target.wants"
ln -snf /usr/lib/systemd/system/getty@.service \
    "${TARGET_DIR}/etc/systemd/system/getty.target.wants/getty@tty1.service"

# ─── Decompress kernel modules ────────────────────────────────────────────────
# The RPi kernel compresses modules as .ko.xz; decompress them here using the
# host xz so plain modprobe works without requiring kmod xz support on target.
...
# Enable gpsd.socket so gpsd starts on first client connection (port 2947).
...
# wpa_supplicant for wlan0 (config at /etc/wpa_supplicant/wpa_supplicant-wlan0.conf)
if [ -f "${TARGET_DIR}/usr/lib/systemd/system/wpa_supplicant@.service" ]; then
    ln -snf /usr/lib/systemd/system/wpa_supplicant@.service \
        "${TARGET_DIR}/etc/systemd/system/multi-user.target.wants/wpa_supplicant@wlan0.service"
fi

# systemd-networkd: manages wlan0 DHCP (see etc/systemd/network/wlan0.network)
...
# systemd-resolved: DNS resolution; /etc/resolv.conf → resolved's upstream list
...
# systemd-timesyncd: NTP client (config at /etc/systemd/timesyncd.conf)
...
# Keep root home out of the rootfs proper so a read-only root mount works later.
...
# Tiles are cached in maps-cache/ at the project root (survives make clean).
# On the first build this takes ~3.5 hours; subsequent builds skip all tiles via
# --resume and complete instantly.
